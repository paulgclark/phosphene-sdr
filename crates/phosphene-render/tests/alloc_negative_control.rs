...
//! D-121 mandatory negative control, a standing CI guard (D-122): design B
//! keeps the process-wide counting allocator, so an allocation made on a
//! thread standing in for the path under test — not the thread running the
//! assertion — must still be visible inside the counted window. This test
//! is `#[should_panic]` and passes exactly when that panic fires. If it
//! ever passes *without* panicking, design B has quietly become a
//! thread-local gate (the rejected design A): a real per-frame allocation
//! handed to a worker thread could then hide from `alloc_trace` /
//! `alloc_waterfall` undetected, and `cargo test` catches that as a failure
